...
const base = `${(process.env.SUPABASE_URL || 'https://bewxefxjbvbumajflhfw.supabase.co').replace(/\/$/, '')}/rest/v1/analytics_events`;
const key = () => process.env.SUPABASE_SERVICE_ROLE_KEY || '';

export async function GET() {
  try { await requireAdmin(); } catch { return NextResponse.json({ message: 'Admin authentication required.' }, { status: 401 }); }
  if (!key()) return NextResponse.json({ message: 'Analytics database access is not configured.' }, { status: 503 });
  try {
    const since = new Date(Date.now() - 30 * 86400000).toISOString();
    const response = await fetch(`${base}?select=event_name,visitor_id,path,created_at&created_at=gte.${encodeURIComponent(since)}&order=created_at.asc&limit=10000`, { headers: { apikey: key(), Authorization: `Bearer ${key()}` }, cache: 'no-store' });
    ...
    for (let i = 29; i >= 0; i--) {
      ...
      const rows = events.filter((event) => String(event.created_at).slice(0, 10) === day);
      const dayVisitors = new Set();
      const item = { date: day, visits: 0, uniqueVisitors: 0, sessions: 0, accounts: 0, cvs: 0, exports: 0 };
      for (const event of rows) {
        if (event.event_name === 'page_view') { item.visits++; dayVisitors.add(event.visitor_id); visitors.add(event.visitor_id); }
        if (event.event_name === 'session_start') item.sessions++;
        if (event.event_name === 'account_created') item.accounts++;
        if (event.event_name === 'cv_created') item.cvs++;
        if (event.event_name === 'cv_exported') item.exports++;
      }
    ...
    const pageCounts = {};
    for (const event of events) if (event.event_name === 'page_view') pageCounts[event.path] = (pageCounts[event.path] || 0) + 1;
