...
const KEY = 'ai-cv-builder-layout-v2';
...
function snapshot(): LayoutMap {
  ...
  document.querySelectorAll<HTMLElement>('.resume-main,.resume-side').forEach(parent => {
    ...
    Array.from(parent.children).forEach((el, index) => { const key = keyOf(el); if (key) result[key] = { column, order: index }; });
  });
  return result;
}
function applyLayout(layout: LayoutMap) {
  ...
  paper.querySelectorAll<HTMLElement>('.resume-section').forEach(el => { const key = keyOf(el); if (key) nodes.set(key, el); });
  Object.entries(layout).forEach(([key, item]) => { const el = nodes.get(key); const parent = item.column === 'side' ? side : main; if (el && el.parentElement !== parent) parent.appendChild(el); });
  [main, side].forEach(parent => Array.from(parent.children)
...
export default function CVInteractions() {
  ...
  const [contact, setContact] = useState<'left' | 'center' | 'right'>('left');
  ...
  useEffect(() => {
    ...
    const savedContact = (localStorage.getItem(`${KEY}-contact-${cvId()}`) || 'left') as 'left' | 'center' | 'right';
    ...
    const apply = () => {
      ...
      if (!paper) return;
      applying = true;
      paper.style.setProperty('--cv-editor-zoom', String((savedZoom >= 70 && savedZoom <= 140 ? savedZoom : 100) / 100));
      document.body.dataset.cvContact = savedContact;
  ...
  useEffect(() => {
    ...
    if (!root) return;
    let dragging = false;
    const makeDraggable = () => root.querySelectorAll<HTMLElement>('.resume-section').forEach(el => {
      el.draggable = true;
      el.dataset.cvKey = keyOf(el);
    });
    const onDragStart = (event: Event) => {
      const ev = event as DragEvent;
      const target = (ev.target as HTMLElement).closest('.resume-section') as HTMLElement | null;
      if (!target) return;
      dragging = true;
      target.classList.add('cv-dragging');
      ev.dataTransfer?.setData('text/cv-section', keyOf(target));
    ...
    const onDrop = (event: Event) => {
      ...
      root.querySelectorAll('.cv-drop-target,.cv-dragging').forEach(x => x.classList.remove('cv-drop-target', 'cv-dragging'));
      writeLayout(snapshot());
      applyLayout(readLayout());
      dragging = false;
    };
    const onDragEnd = () => { if (dragging) { dragging = false; root.querySelectorAll('.cv-drop-target,.cv-dragging').forEach(x => x.classList.remove('cv-drop-target', 'cv-dragging')); } };
    makeDraggable();
    ...
    return () => { root.removeEventListener('dragstart', onDragStart); root.removeEventListener('dragover', onDragOver); root.removeEventListener('drop', onDrop); root.removeEventListener('dragend', onDragEnd); observer.disconnect(); };
  }, [ready, dragMode]);

  if (!ready) return null;
  const changeZoom = (delta: number) => setZoom(value => Math.max(70, Math.min(140, value + delta)));
  const reset = () => { localStorage.removeItem(storageKey()); applyLayout({}); setZoom(100); setContact('left'); };
  ...
  return <div className="cv-floating-tools" aria-label="CV page controls">
    ...
    <span className="cv-tool-divider" />
    <button title={dragMode ? 'Disable drag' : 'Enable drag'} className={dragMode ? 'active' : ''} onClick={() => setDragMode(value => !value)}><GripVertical size={14} /></button>
