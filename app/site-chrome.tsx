...
export default function SiteChrome() {
  ...
  if (isEditor) return (
    <div className="public-dock" aria-label="AI CV Builder public pages">
      ...
      <Link href="/profile"><img src={avatar} alt=""/> Profile</Link><Link href="/feedback"><Star size={12}/> Feedback</Link><Link href="/contact">Contact</Link>
  ...
  return <header className="public-nav">
    ...
    <div className="public-actions"><span className="version-chip">v{APP_VERSION}</span><Link href="https://github.com/bc878vu" target="_blank" rel="noreferrer" aria-label="GitHub"><Github size={17}/></Link><Link href="mailto:a.m.a63425@gmail.com" aria-label="Email"><Mail size={17}/></Link><Link href="/profile" aria-label="Profile"><UserRound size={17}/></Link><button onClick={()=>setOpen(x=>!x)} aria-label="Open navigation menu" className="mobile-menu">{open?<X size={18}/>:<Menu size={18}/>}</button></div>
    {open&&<div className="mobile-nav">{links.map(([href,label])=><Link key={href} href={href} onClick={()=>setOpen(false)}>{label}</Link>)}<Link href="/dashboard" onClick={()=>setOpen(false)}>My CVs</Link><Link href="/" onClick={()=>setOpen(false)}>Open Editor</Link><span>Version {APP_VERSION}</span></div>}
  </header>;
}
